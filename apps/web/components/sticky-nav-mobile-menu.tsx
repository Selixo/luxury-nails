...
interface StickyNavMobileMenuProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function StickyNavMobileMenu({
  open,
  onOpenChange,
}: StickyNavMobileMenuProps) {
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex h-full flex-col">
            ...
            {NAV_LINKS.map((link, i) => (
              <li key={link.href}>
                ...
                >
                  ...
                  <span
                    aria-hidden="true"
                    className="absolute bottom-0 left-0 h-px w-0 bg-gold/30 transition-all duration-300 group-hover:w-full group-focus-visible:w-full"
                  />
                </Link>
              </li>
            ))}
          ...
          >
            <Link href="#rezerwacja" onClick={() => onOpenChange(false)}>
              Zarezerwuj wizytę
            </Link>
          </Button>
        </div>
